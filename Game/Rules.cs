...
namespace Game
{
    ...
    class Rules
    {
        ...
        {
            Console.WriteLine("Your move:{0}\nComputer move:{1}", arg_cmd[selection - 1], arg_cmd[randomChoice]);
            if (selection == randomChoice + 1) Console.WriteLine("Draw");
            if ((selection < randomChoice + 1) && ((selection + arg_cmd.Length / 2.0) < (randomChoice + 1))) { Console.WriteLine("You lose"); }
            if ((selection < randomChoice + 1) && (selection + arg_cmd.Length / 2.0) > (randomChoice + 1)) { Console.WriteLine("You Win"); }
            if ((selection > randomChoice + 1) && (((randomChoice + 1)+ arg_cmd.Length / 2.0) < selection)) { Console.WriteLine("You Win"); }
            if ((selection > randomChoice + 1) && (((randomChoice + 1) + arg_cmd.Length / 2.0) > selection)) { Console.WriteLine("You lose"); }
        ...
        public static void New_Game(string[] arg_cmd)
        {
            Console.WriteLine("New Game");
                ...
                int randomChoice = RandomNumberGenerator.GetInt32(arg_cmd.Length);
                Message = "Computer move:" + arg_cmd[randomChoice];
